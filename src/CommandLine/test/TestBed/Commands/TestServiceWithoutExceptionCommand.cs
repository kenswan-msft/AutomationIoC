...
internal class TestServiceWithoutExceptionCommand : IAutomationCommand
{
    ...
    {
        Option<string> passedInOption = new(name: "--test") { Description = "Description of test." };
        ...
        command.SetAction((parseResult, automationContext) =>
        {
            ...
            TestExecution(automationContext.ServiceProvider.GetRequiredService<ITestService>(), passedInOptionString);
        });
    }
    ...
    // {
    //     var appSettings = new Dictionary<string, string> { [TestService.CONFIG_KEY] = TestService.CONFIG_VALUE };
    //
